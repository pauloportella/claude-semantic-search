...
from src.cli import SemanticSearchCLI, cli
...
class TestSemanticSearchCLI:
    ...
    def test_index_conversations_success(
        self,
        mock_add_chunks,
        mock_generate_embeddings,
        mock_chunk_conversation,
        mock_parse_file,
        mock_scan_claude_projects,
    ):
        """Test successful conversation indexing."""
        # Mock dependencies
        mock_scan_claude_projects.return_value = [
            Path("test1.jsonl"),
            Path("test2.jsonl"),
        ]
        ...
        mock_chunk = Chunk(
            id="test_chunk", text="Test chunk text", metadata={"test": "metadata"}
        )
    ...
    @patch("src.cli.SemanticSearchCLI.scan_claude_projects")
    @patch("src.parser.JSONLParser.parse_file")
    def test_index_conversations_parse_error(
        self, mock_parse_file, mock_scan_claude_projects
    ):
    ...
    def test_search_conversations_success(
        self, mock_search, mock_generate_single_embedding
    ):
        ...
        self.cli_instance.embedder = Mock()
        self.cli_instance.embedder.is_model_loaded = True
        self.cli_instance.embedder.generate_single_embedding = (
            mock_generate_single_embedding
        )
...
class TestCLICommands:
    ...
    def test_index_command_success(
        self, mock_index_conversations, mock_scan_claude_projects
    ):
    ...
    def test_search_command_success(self, mock_search_conversations):
        ...
        result = self.runner.invoke(
            cli, ["--data-dir", self.temp_dir, "search", "test query"]
        )
    ...
    def test_search_command_json_output(self, mock_search_conversations):
        ...
        result = self.runner.invoke(
            cli, ["--data-dir", self.temp_dir, "search", "test query", "--json"]
        )
    ...
    def test_search_command_with_filters(self, mock_search_conversations):
        ...
        assert (
            args[1]["project_name"] == "test_project"
    ...
    def test_search_command_full_content(self, mock_search_conversations):
        ...
        long_text = (
            "This is a very long piece of text that would normally be truncated at 200 characters but should be displayed in full when the --full-content flag is used. "
            * 3
        )
        ...
        result = self.runner.invoke(
            cli, ["--data-dir", self.temp_dir, "search", "test query", "--full-content"]
        )
        ...
        assert (
            "..." not in result.output.split(long_text)[1].split("\n")[0]
        )  # No ... after the text
    ...
    def test_search_command_truncated_content(self, mock_search_conversations):
        ...
        long_text = (
            "This is a very long piece of text that should be truncated at 200 characters when the --full-content flag is not used. "
            * 3
        )
    ...
    def test_search_command_chunk_id_success(self, mock_cli_class):
        ...
        result = self.runner.invoke(
            cli,
            ["--data-dir", self.temp_dir, "search", "", "--chunk-id", "test_chunk_123"],
        )
    ...
    def test_search_command_related_to_same_session(self, mock_cli_class):
        ...
        mock_instance.storage.get_chunks_by_session.assert_called_once_with(
            "session_456"
        )
    ...
    def test_search_command_related_to_no_session(self, mock_cli_class):
        ...
        assert result.exit_code == 1
        assert (
            "❌ Reference chunk has no session ID: sessionless_chunk" in result.output
        )
...
class TestCLIIntegration:
    ...
    def test_cli_search_no_results(self, mock_search_conversations):
        ...
        result = self.runner.invoke(
            cli, ["--data-dir", self.temp_dir, "search", "nonexistent"]
        )
