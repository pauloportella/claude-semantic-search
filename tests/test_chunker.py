    ...
    def create_test_conversation(self) -> Conversation:
        ...
        messages = [
            ...
            Message(
                ...
                role="assistant",
                has_code=True,
                tool_calls=[{"name": "python", "input": "def sort_dicts(data, key):\n    return sorted(data, key=lambda x: x[key])"}]
            ),
            Message(
                uuid="msg-005",
...
```"""
    ...
    def test_identify_context_segments(self):
        ...
        messages = [
            ...
            Message(uuid="2", content="Response 1", timestamp=base_time + timedelta(seconds=30), role="assistant"),
            Message(uuid="3", content="Follow up", timestamp=base_time + timedelta(minutes=1), role="user"),
            Message(uuid="4", content="Response 2", timestamp=base_time + timedelta(minutes=1, seconds=30), role="assistant"),
            # Large time gap
            Message(uuid="5", content="Now let's discuss something different", timestamp=base_time + timedelta(hours=1), role="user"),
            Message(uuid="6", content="New topic response", timestamp=base_time + timedelta(hours=1, seconds=30), role="assistant"),
        ]
    ...
    def test_split_large_chunk(self):
        ...
        assistant_msg = Message(
            uuid="a1",
            content="Very long response that exceeds the maximum chunk size limit. " * 50,
            timestamp=datetime.now(),
            role="assistant"
        )
        
        # Create text that exceeds max_chunk_size and can be split by words
        long_text = "word " * 250  # 250 * 5 = 1250 chars, exceeds max_chunk_size of 1000
    ...
    def test_single_message_conversation(self):
        ...
        assert len(chunks) == 0 or all(chunk.metadata["chunk_type"] != "qa_pair" for chunk in chunks)
    ...
    def test_min_chunk_size_filtering(self):
        ...
        short_conversation = Conversation(
            ...
            messages=[
                ...
                Message(uuid="2", content="Hello", timestamp=datetime.now(), role="assistant")
            ],
            project_name="test",
            file_path="/test",
            created_at=datetime.now(),
        ...
        assert len(chunks) == 0 or all(len(chunk.text) >= config.min_chunk_size for chunk in chunks)