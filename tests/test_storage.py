...
from src.storage import HybridStorage, StorageConfig, SearchConfig, SearchResult
...
class TestHybridStorage:
    ...
    def test_backup_and_restore(self):
        ...
        assert (backup_path / self.config.index_name).exists()  # Should exist since we added chunks
    ...
    def test_matches_filters(self):
        ...
        # Test exact match
        assert self.storage._matches_filters(chunk_data, {"project_name": "test_project"})
        assert not self.storage._matches_filters(chunk_data, {"project_name": "other_project"})
        ...
        # Test list filters
        assert self.storage._matches_filters(chunk_data, {"chunk_type": ["qa_pair", "code_block"]})
        assert not self.storage._matches_filters(chunk_data, {"chunk_type": ["tool_usage"]})
