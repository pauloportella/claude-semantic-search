...
class TestEndToEndWorkflow:
    ...
    def test_complete_workflow(self):
        ...
        if len(chunks) == 0:
            pytest.skip(
                "Chunking returned no chunks - this is expected behavior for short conversations"
            )
        ...
        with patch("src.embeddings.SentenceTransformer") as mock_transformer:
            ...
            embedder = EmbeddingGenerator(
                EmbeddingConfig(model_name="all-mpnet-base-v2", cache_dir=self.temp_dir)
            )
            embedder.load_model()
            ...
            assert all(
                len(embedding) == 3 for embedding in embeddings
            )  # 3D for testing
    ...
    def test_storage_and_search_workflow(self):
        ...
        storage = HybridStorage(
            StorageConfig(data_dir=self.temp_dir, embedding_dim=4)  # 4D for testing
        )
        storage.initialize()
        ...
        # Test search
        search_config = SearchConfig(top_k=3, include_metadata=True, include_text=True)
        ...
        assert all(
            result.metadata.get("project_name") == "test-project"
    ...
    def test_search_relevance(self):
        ...
        # Initialize storage
        storage = HybridStorage(StorageConfig(data_dir=self.temp_dir, embedding_dim=4))
        storage.initialize()
