...
class ConversationChunker:
    ...
    def _is_hook_message(self, message: Message) -> bool:
        ...
        hook_patterns = [".claude/hooks/", "hook:", "hooks.json", "claude code hook"]
            ...
            if messages[i].role == "user" and messages[i + 1].role == "assistant":
                ...
                if len(chunk_text) > self.config.max_chunk_size:
                    ...
                    sub_chunks = self._split_large_chunk(
                        chunk_text, user_msg, assistant_msg
                    )
    ...
    def _create_context_chunks(self, conversation: Conversation) -> List[Chunk]:
        ...
        for segment in segments:
            ...
            if (
                ...
                and len(chunk_text) >= self.config.min_chunk_size
            ):
                chunk = self._create_chunk(
                    chunk_text,
    ...
    def _get_context(
        self, messages: List[Message], current_idx: int, context_size: int
    ) -> str:
    ...
    def _identify_context_segments(
    ...
    def _extract_code_blocks(self, content: str) -> List[Dict[str, Any]]:
        ...
        for match in matches:
            ...
            code_blocks.append(
                {"language": language, "code": code, "raw": match.group(0)}
            )
        ...
        for match in inline_matches:
            ...
            if len(code) > 20:  # Only longer inline code
                code_blocks.append(
                    {"language": "text", "code": code, "raw": match.group(0)}
                )

        return code_blocks

    def _split_large_chunk(
        self, chunk_text: str, user_msg: Message, assistant_msg: Message
    ) -> List[Chunk]:
        ...
        if len(paragraphs) == 1 or any(
            len(p) > self.config.max_chunk_size for p in paragraphs
        ):
