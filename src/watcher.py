        ...
        try:
            ...
            for dir_path in dirs_to_scan:
                ...
                if dir_files:
                    ...
                    stats = self.cli_instance.index_conversations(dir_files, force=False)
...
class ConversationWatcher:
    ...
    def start_watching(self, claude_dir: str = "~/.claude/projects"):
        ...
        if not claude_path.exists():
            raise FileNotFoundError(f"Claude projects directory not found: {claude_path}")
    ...
    def get_status(self) -> dict:
        ...
        return {
            ...
            "watching_path": self.observer.emitters[0].watch.path if self.observer.emitters else None,
            "pending_files": len(self.handler.pending_files),
            "last_trigger_time": self.handler.last_trigger_time.isoformat() if self.handler.last_trigger_time else None,
...
def stop_daemon(data_dir: str = "./data"):
...
def daemon_status(data_dir: str = "./data"):
    """Check daemon status."""
    watcher = ConversationWatcher(data_dir)
