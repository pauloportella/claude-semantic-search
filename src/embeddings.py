...
class EmbeddingGenerator:
    ...
    def load_model(self) -> None:
        ...
        try:
            ...
            self.model = SentenceTransformer(
                self.config.model_name, cache_folder=cache_dir
            )
            ...
            if (
                self.config.use_gpu
                and self.config.auto_batch_size
                ...
                if self._gpu_capability.gpu_memory_free:
                    ...
                    optimal_batch = calculate_optimal_batch_size(
                        memory_gb, backend=backend
                    )
                    self.config.batch_size = optimal_batch
                    self.logger.info(
                        f"Auto-adjusted batch size for GPU ({backend}): {optimal_batch}"
                    )
            ...
            self.logger.info(
                f"Model loaded successfully on {device_info}. Embedding dimension: {self._embedding_dim}"
            )
    ...
    def _generate_embeddings_batch(self, texts: List[str]) -> List[np.ndarray]:
        ...
        for i, text in enumerate(texts):
            if text is None:
                ...
                self.logger.warning(
                    f"Skipping chunk {i}: text is not string (type: {type(text)})"
                )
                validated_texts.append(str(text) if text else "")  # Convert to string
            elif not text.strip():
                self.logger.warning(
                    f"Skipping chunk {i}: text is empty or whitespace only"
                )
    ...
    def benchmark_model(
        self, test_texts: List[str], warmup_runs: int = 3
    ) -> Dict[str, Any]:
        ...
        for _ in range(warmup_runs):
            self.model.encode(
                test_texts[: min(5, len(test_texts))], show_progress_bar=False
            )
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        info = {
            ...
            "gpu_available": (
                self._gpu_capability.can_use_gpu if self._gpu_capability else False
            ),
        }
